function [Face_Topo,Face_Lateral,Vertices] = CriaElo(Num_Faces,Raio,Altura)
    ...
    for L1 = 1:Linha
        if L1<= Linha/2
            if L1 <= Linha/4
                Vertices(L1,:) = [VC(L1,1)+(Raio*0.6),VC(L1,2),VC(L1,3)];
            elseif L1 == (Linha/4)+1
            ...
            if L1 <= 3*Linha/4
                Vertices(L1,:) = [VC(L1-1,1)+(Raio*0.6),VC(L1-1,2),VC(L1-1,3)];
